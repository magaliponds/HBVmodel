...
# writedlm("/home/sarah/Master/Thesis/Results/Projektionen/"*Catchment_Name*"/Comparison_Real_Proj/Discharge/Discharge_obs_85_13.csv", Observed_Discharge)
# writedlm("/home/sarah/Master/Thesis/Results/Projektionen/"*Catchment_Name*"/Comparison_Real_Proj/Discharge/Discharge_mod__measured_data_85_13.csv", All_Discharges)
...
function plot_hydrographs(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
        for i in 1:Nr_Years
                ...
                if current_year == 1990
                        ...
                        hline!([0], color="black")
                        temperature = plot!()
                        plot(discharge, precipitation, temperature, layout= (3,1), legend = false, size=(3000,1800), left_margin = [5mm 0mm], bottom_margin = 20px, xrotation = 60, yguidefontsize=20, xtickfont = font(20), ytickfont = font(20), dpi=300)
                        ...
                        #savefig("/home/sarah/Master/Thesis/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_All_"*string(current_year)*"_with_Prec_other_Temp_font.png")
                end
        end
end
...
function plot_hydrographs_final_report(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
        ...
        for i in 1:Nr_Years
                ...
                ylabel!("Discharge [mm/d]")
                title!("Pitztal "*string(current_year), titlefont=font(22))
                ...
                discharge = plot!()
                #plot!(p, Timeseries[indexfirstday:indexlasttday], Total_Precipitation[indexfirstday:indexlasttday], label="Observed Rainfall",size=(1800,1000), color = ["blue"], linewidth = 3)
                plot(Timeseries[indexfirstday:indexlasttday], Total_Precipitation[indexfirstday:indexlasttday], label="Measured Precipitation",size=(1800,1000), color = ["blue"], linewidth = 2, framestyle = :box, gridalpha = 0.3)
                ylabel!("Precipitation [mm/d]")
                xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                precipitation = plot!()
                # plot Temperature
                plot(Timeseries[indexfirstday:indexlasttday], Temperature_Mean_Elevation[indexfirstday:indexlasttday], label="Measured Temperature",size=(1800,1000), color = ["red"], linewidth = 2, framestyle = :box, gridalpha = 0.3)
                ylabel!("Temperature [°C]")
                xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                temperature = plot!()
                if i == 1
                ...
                savefig("/home/sarah/Master/Thesis/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_"*string(current_year)*"_final.png")
        end
end
...
plot_hydrographs_final_report(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, 27, Area_Catchment_Pitztal, 300)
...
#plot_hydrographs(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, 20, Area_Catchment_Defreggental, 300)
